...
namespace ModChart
{
    static class BeatmapCompressor
    {
        ...
        public static BeatMap.CustomData.CustomEvents.Data SimplifyAllPointDefinitions(this BeatMap.CustomData.CustomEvents.Data animation)
        {
            foreach (var property in typeof(BeatMap.CustomData.CustomEvents.Data).GetProperties().Where(p => new string[] { "_color", "_rotation", "_localRotation", "_position", "_definitePosition", "_dissolve", "_dissolveArrow", "_scale" }.Any(name => name == p.Name)))
            {
                dynamic customprop = property.GetValue(animation);
                if ((customprop is JsonElement || customprop != null) && isArray(customprop))
                {
