...
namespace ScuffedWalls.Functions
{
    ...
    {
        internal const string NAME = "CloneWorkspace";
        public override void Run()
        {
            ...
            if (Type.Any(t => t == 0))
            {
                ...
                if (filtered.Count() > 0) ConsoleOut("_obstacle", filtered.Count(), Time, NAME);
            }
            if (Type.Any(t => t == 1))
            {
                ...
                if (filtered.Count() > 0) ConsoleOut("_note", filtered.Count(), Time, NAME);
            }
            if (Type.Any(t => t == 2))
            {
                ...
                if (filtered.Count() > 0) ConsoleOut("_event", filtered.Count(), Time, NAME);
            }
            if (Type.Any(t => t == 3))
            {
                ...
                foreach (var item in cloned.CustomData) if (item.Value is IEnumerable<object> array && array.Count() > 0) ConsoleOut($"_customData.{item.Key}", array.Count(), Time, NAME);
            }
